    ...
    def on_release(key):
        if key == Key.up: # si le bouton directionnel haut est relâché
           ...
           for i in range (0,20):
               ser.write(b'1')
        if key == Key.down:
            print("tilt stop") # si le bouton directionnel bas est relâché
            for i in range (0,20):
                ser.write(b'1')
...
for line in gphoto2_data.split('\n'):
    ...
    if line_list[0] == "Choice":
        ...
        my_key, my_value = line_list[1].split(' f/')
...
def reinitialisation():
    ...
    os.system('gphoto2 --set-config-index f-number=0')
...
def callback_wb(*args):
    ...
    if active :
        ...
        if w.get()=='Tungstène':
            ...
            os.system('gphoto2 --set-config-value whitebalance="Automatic"')
        elif w.get()=='Nuageux':
            ...
            os.system('gphoto2 --set-config-value whitebalance="Preset"')
        time.sleep(0.5)
        os.system('gphoto2 --capture-movie --stdout>fifo.mjpg &')
        #messagebox.showinfo("Liveview actif", "Veuillez désactiver le liveview avant toute autre opération")
    else:
